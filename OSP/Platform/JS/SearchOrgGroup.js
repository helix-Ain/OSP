function setOrgGroupComboGrid(ci_strID, ci_strValue, ci_strText) {
    ...
    if (m_combo) {
        ...
        $("#txt_" + ci_strID).searchbox({
            searcher: function(ci_strValue, ci_strName) {
                searchOrgGroup(ci_strID, ci_strValue);
            },
            prompt: "请输入组织组中文名",
            width: 155,
            value: ""
        ...
        if (m_grid == null || m_grid.datagrid("getData").total == 0) {
            m_combo.combogrid({
                ...
                url: getInterfaceName("DcCdOrgGroupManage"),
                ...
                pageSize: 5,
                columns: [[
                    { field: 'cGroupCode', title: '组织组编号', width: 60, sortable: true },
                    { field: 'cGroupChiName', title: '中文名', width: 100, sortable: true }
                ]]
            });
        }
        else {
            searchOrgGroup(ci_strID);
        }
...
function searchOrgGroup(ci_strID, ci_strValue) {
