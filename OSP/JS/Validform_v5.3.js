/*
	...
	$(".demoform").Validform({//$(".demoform")指明是哪一表单需要验证,名称需加在form表单上;
		...
		tiptype:1, //可选项 1=>pop box,2=>side tip(parent.next.find; with default pop),3=>side tip(siblings; with default pop),4=>side tip(siblings; none pop)，默认为1，也可以传入一个function函数，自定义提示信息的显示方式（可以实现您想要的任何效果，具体参见demo页）;
		...
		callback:function(data){
			...
			//您也可以在ajax_post.php文件返回更多信息在这里获取，进行相应操作；
    ...
    Validform.util = {
        ...
        getValue: function(obj) {
            ...
            if (obj.is(":radio")) {
                ...
                inputval = inputval === undef ? "" : inputval;
            } else if (obj.is("select.combo-f")) {
                inputval = obj.parents("td").find("span>input.combo-value").val();
                inputval = inputval === undef ? "" : inputval;
            } else {
                inputval = obj.val();
            }
        ...
        enhance: function(tiptype, usePlugin, tipSweep, addRule) {
            ...
            curform.find("[focustip]").each(function() {
                //已经绑定事件时跳过;
                if (this.validform_inited == "inited") { return true; }
                this.validform_inited = "inited";

                var focustip = $(this).attr("focustip");
                var altercss = $(this).attr("altercss");
                $(this).focus(function() {
                    $(this).parent().find(".Validform_checktip").removeClass("Validform_wrong Validform_right Validform_loading");
                    $(this).parent().find(".Validform_checktip").addClass("Validform_focustip");
                    $(this).parent().find(".Validform_checktip").html(focustip);
                });
            });
        ...
        cssctl: function(obj, status) {
            switch (status) {
                case 1:
                    obj.removeClass("Validform_right Validform_wrong Validform_focustip").addClass("Validform_checktip Validform_loading"); //checking;
                    break;
                case 2:
                    obj.removeClass("Validform_wrong Validform_loading Validform_focustip").addClass("Validform_checktip Validform_right"); //passed;
                    break;
                case 4:
                    obj.removeClass("Validform_right Validform_wrong Validform_loading Validform_focustip").addClass("Validform_checktip"); //for ignore;
                    break;
                default:
                    obj.removeClass("Validform_right Validform_loading Validform_focustip").addClass("Validform_checktip Validform_wrong"); //wrong;
            }
        },
        ...
        resetForm: function() {
            ...
            brothers.find(".Validform_checktip").removeClass("Validform_wrong Validform_right Validform_loading Validform_focustip").text("");
            brothers.find(".Validform_error").removeClass("Validform_error");
            brothers.find("[datatype]").removeData("cked").removeData("dataIgnore");
            brothers.find("[focustip]").each(function() {
                var focustip = $(this).attr("focustip");
                $(this).parent().find(".Validform_checktip").html(focustip);
            });
