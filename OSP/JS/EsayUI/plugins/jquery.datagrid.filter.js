(function($) {
    ...
    */
    ...
    function doFilter(target, isinit) {
        var name = getPluginName(target);
        var state = $.data(target, name);
        var opts = state.options;
        if (opts.remoteFilter) {
            if (isinit) {
                $(target)[name]('autoSizeColumn');
            }
            else {
                $(target)[name]('load');
            }
    ...
    function init(target, filters) {
        ...
        var opts = state.options;
        if (!opts.filterInit) {
            opts.filterHide = false;
        }
        opts.filterInit = true;
        ...
        */
        function createFilter(frozen) {
            ...
            for (var i = 0; i < fields.length; i++) {
                ...
                input.addClass('datagrid-filter').addClass('TextEdit').attr('name', field);
            ...
            //table.find('.datagrid-header-rownumber').parent().parent().css('height', '27px');
        }
        ...
        function createFilterButton(container, operators) {
            ...
            btn.closest('td[field]').find('input.datagrid-filter').unbind('.filter').bind('keydown.filter', function(e) {
                if (e.keyCode == 13) {
                    if (menu.find('.' + opts.filterMenuIconCls).length > 0) {
                        menu.menu('options').onClick.call(menu, menu.menu('getItem', menu.find('.' + opts.filterMenuIconCls).parent()[0]));
                    }
                    else {
                        menu.menu('options').onClick.call(menu, menu.menu('getItem', menu.find('.menu-item')[0]));
                    }
                }
            });
            return menu;
        ...
        hideFilter: function(jq) {
            return jq.each(function() {
                ...
                if (opts) {
                    ...
                    opts.filterHide = true;
                    $('.datagrid-filter-row').css('display', 'none').parent().parent().css('height', 'auto');
                    $('.datagrid-header').css('height', 'auto')
                    $(this)[name]('resize');
                }
            });
        },
        showFilter: function(jq) {
            return jq.each(function() {
                ...
                if (opts) {
                    ...
                    opts.filterHide = false;
                    $('.datagrid-filter-row').css('display', '');
                    resizeFilter(this);
                    $(this)[name]('resize');
                }
            });
        },
        displayFileter: function(jq) {
            return jq.each(function() {
                var name = getPluginName(this);
                var opts = $.data(this, name).options;
                if (opts) {
